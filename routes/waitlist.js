...
router.post('/join', authenticateToken, validate(joinWaitlistSchema), asyncHandler(async (req, res) => {
  const { event_id, quantity } = req.body;

  try {
    const waitlistEntry = await Waitlist.join(req.user.id, event_id, quantity);
...
router.delete('/:event_id', authenticateToken, asyncHandler(async (req, res) => {
...
// Get waitlist notifications for user
...
// Get waitlist entries for a specific event (admin/event creator only)
router.get('/event/:event_id', authenticateToken, asyncHandler(async (req, res) => {
  const { event_id } = req.params;
  ...
  // TODO: Add permission check for event creator/admin
